...
async function storeScanPrice(keyword, listing) {
  ...
  // Bundles skew single-item medians — never enter the price pool
  if (BUNDLE_PRICE_FILTER.test(listing.title || '')) return;
...
function buildCacheVerdict(listingPrice, priceData) {
  ...
  // AU market clears ~12% below asking price — use realistic sell price as the flip anchor
  const marketSellPrice = Math.round(median * 0.88);
  const roi = marketSellPrice > 0 ? Math.round(((marketSellPrice - listingPrice) / listingPrice) * 100) : 0;
  const estimatedProfit = Math.round(marketSellPrice - listingPrice);

  let verdict, oneLiner, dealScore;
  if (roi >= 40) {
    verdict   = 'STEAL';
    oneLiner  = `Listed ${roi}% below realistic sell price — exceptional flip`;
    dealScore = 95;
  } else if (roi >= 25) {
    verdict   = 'GOOD DEAL';
    oneLiner  = `Listed ${roi}% below realistic sell price — strong flip`;
    dealScore = 80;
  } else if (roi >= 12) {
    verdict   = 'GOOD DEAL';
    ...
    verdict   = 'PASS';
    oneLiner  = `Listed ${Math.abs(roi)}% above realistic sell price — negotiate hard or pass`;
    dealScore = 20;
  }
  ...
  return {
    ...
    walkAwayPrice:       Math.round(marketSellPrice * 0.95),
    estimatedProfit:     Math.max(0, estimatedProfit),
    roiPercent:          roi,
    dataPoints:          count,
    source,
    low, median, high,
    negotiationScript:   `Similar listings realistically sell for around $${marketSellPrice} — would you take $${Math.round(listingPrice * 0.85)}?`,
  };
}
...
function buildVehicleVerdict(listingPrice, priceSource, mileage) {
  ...
  // AU market clears ~12% below asking price — use realistic sell price as the flip anchor
  const marketSellPrice = Math.round(marketMedian * 0.88);
  const roi             = marketMedian > 0 ? Math.round(((marketSellPrice - listingPrice) / listingPrice) * 100) : 0;
  const estimatedProfit = Math.max(0, Math.round(marketSellPrice - listingPrice));
  ...
  if (roi >= 40) {
    ...
    oneLiner = `Listed ${Math.abs(roi)}% above realistic market — negotiate hard or pass`;
  }
  ...
  return {
    ...
    median: marketMedian,
    high:   marketHigh,
    negotiationScript: `Similar ${carLabel}s realistically sell for around $${marketSellPrice.toLocaleString()} — would you take $${Math.round(listingPrice * 0.82).toLocaleString()}?`,
  };
}
...
// Bundles pollute single-item price medians — excluded from the price pool at write time
const BUNDLE_PRICE_FILTER = /\b(bundle|lot of|job.?lot|x ?\d{1,2} games?|\+ games?|with games?|plus games?|collection of|\d+ items?|set of|pack of|combo|joblot|wholesale|mixed lot)\b/i;
  ...
  while (true) {
    ...
    for (const row of rows) {
      try {
        ...
        // Use product-level stats when available — more precise than broad keyword median
        let median = row.median_price || null;
        let p25    = row.p25_price    || null;
        let p75    = row.p75_price    || null;
        let priceSource = 'keyword';
        if (row.extracted_product) {
          try {
            const pKey = row.extracted_product.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
            const ps = await pool.query(
              'SELECT median_price, p25_price, p75_price, sample_count FROM product_price_stats WHERE product_key = $1',
              [pKey]
            );
            if (ps.rows.length && ps.rows[0].sample_count >= 5) {
              median      = ps.rows[0].median_price;
              p25         = ps.rows[0].p25_price;
              p75         = ps.rows[0].p75_price;
              priceSource = `product (n=${ps.rows[0].sample_count})`;
            }
          } catch (_) {}
        }
...
- Market median (${priceSource}): ${median ? '$'+median+' AUD' : 'unknown'} — NOTE: this is asking price; items typically sell ~12% below asking
- Market p25/p75: ${p25 ? '$'+p25 : '?'} / ${p75 ? '$'+p75 : '?'}
...
- AU items typically SELL for 12% below their asking price — factor this into your margin estimate
- Only approve if clearly undervalued vs what AU Facebook Marketplace items actually SELL FOR (not asking price)
- Minimum $150 net profit after all costs (buy, prep/clean, relist, time)
...
- AU items typically SELL for 12% below asking — base your rating on what they actually SELL FOR, not listing price
- Only green/rainbow if REAL profit margin after all costs (buy, prep/clean, relist, time)
- rainbow = exceptional flip, 35%+ below realistic AU sell price
- green = solid deal, 20-35% below realistic AU sell price
- yellow = fair price
